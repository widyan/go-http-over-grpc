...
import (
	...
	"github.com/widyan/go-http-over-grpc/third_party"
)
...
func Run(logger *logrus.Logger, dialAddr string) error {
	...
	err = pb.RegisterTestHandler(ctx, gwmux, conn)
	if err != nil {
		return fmt.Errorf("failed to register gateway: %w", err)
	}

	err = pb.RegisterTestServiceLagiHandler(ctx, gwmux, conn)
	if err != nil {
		return fmt.Errorf("failed to register gateway: %w", err)
	}
